...
/*
    처음 문제 풀때의 접근 방법은
    모음을 찾을 때 이중 포문을 사용해서 모음을 하나라도 갖고 있으면
    true 를 리턴하는 식으로 작성했는데, 그 방식은 O(N^2) 일뿐만아니라,
    자음의 갯수를 카운팅하는 것에 있어서 오류를 내포하고 있었다.

    또한 start 가 C 를 넘어서는 안된다는 것과
    필요한 문자의 갯수가 현재 소유하고 있는 문자의 갯수보다 많은 경우 오류라는 것
    이 두가지 탈출 조건을 더 추가해야된다는게 중요한 부분이었다.

    백트래킹의 핵심은 
    1. 탈출조건을 명확히해야한다. (빠지는 부분이 있어선 안됨)
    2. 탐색을 한다 (DFS 를 주로 사용)
*/