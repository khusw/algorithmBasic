...
int t;
int dp[101][10001];  // dp[i][j] = k : i 번노드 까지 j 의 비용으로 갔을때 최소시간 j
vector<Info> adj[101];
...
int dijkstra(int n, int m) {
    ...
    while (!pq.empty()) {
        ...
        for (int i = 0; i < adj[_destination].size(); ++i) {
            ...
            if (_cost + next.cost > m) continue;
            if (dp[next.destination][_cost + next.cost] <= _time + next.time) continue;
